...
func _physics_process(delta):
	...
	# When crouching no sprinting is possible
	if Input.is_action_pressed("crouch"):
		curr_speed = CROUCHING_SPEED
		head.position.y = HEAD_POS_DEF + crouching_depth
	else:
		head.position.y = HEAD_POS_DEF
